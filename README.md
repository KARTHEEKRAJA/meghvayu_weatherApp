...
**Design decisions**:
- Historical ranges use Open-Meteo's archive API; near-future ranges use the forecast API — chosen automatically.
...
npm test
```
16 tests cover date-range validation (format, impossible dates like Feb 31, range limits, archive/forecast windows), GPS coordinate parsing, and the insights engine — using Node's built-in `node:test` runner, so no extra dev dependencies.
