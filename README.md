...
**Stack:** Next.js (JavaScript) frontend + **Python (FastAPI)** backend with SQLite, as required for the Full Stack track. An equivalent Node/Express implementation of the same API is included in `backend-node/`: both backends expose identical endpoints and JSON shapes, pass the same 17 test suite, and can be swapped without touching the frontend.

## 🔗 Live demo

- **App:** [meghvayu](https://meghvayu.vercel.app/)
<!-- - **API health check:** [API Health Check for backend-node](https://meghvayu-api.onrender.com/api/health) -->
- **API health check:** [API Health Check for backend](https://meghvayu-api.onrender.com/api/health)
- **API docs (auto generated by FastAPI):** [meghvayu-api.onrender.com/docs](https://meghvayu-api.onrender.com/docs)
- **Demo video:** [Demo Video Link](https://youtu.be/AyEFckeBMoc)
...
- **Production resilience**: every external dependency (weather and geocoding) has retry with backoff, an in memory cache, and an automatic fallback provider added after real rate limit incidents in production (see Debugging notes)
...
## ⭐ What makes MeghVayu stand out

- **Two complete backends, one API contract.** Python/FastAPI (primary) and Node/Express (reference) expose identical endpoints and pass identical 17 test suites, the twin suite is how the Python port was verified.
- **No single point of failure.** Weather and geocoding each have retry with backoff, an in memory cache, and an automatic fallback provider. The app survived two real production rate limit incidents during development without user visible downtime after the fixes.
- **Smart Insights.** A rule based engine turns raw forecast numbers into traveler decisions: pack layers, umbrella day, best outdoor day.
- **Five documented debugging stories.** Real issues found and fixed the international zip code collisions, Excel encoding, native module builds, and shared IP rate limiting written up in Debugging notes below.
- **Condition driven experience.** The theme and ambient particles (rain, snow, stars, lightning) follow the live weather of the searched city, with `prefers-reduced-motion` respected.
- **Reviewer friendly by design.** Runs locally with zero API keys and zero database setup, deployed live with auto generated API docs, and fully documented in this README.

---

## Smart Insights engine

The backend includes a rule based insights engine (`backend/services/insights.py`) that analyzes
...
##### 17 passing unit tests, Python backend (pytest)
![Python tests](screenshots/pytest.png)

##### 17 passing unit tests, Node reference backend (node:test)
![Node tests](screenshots/tests.png)
...
| Geocoding | **OpenStreetMap Nominatim** (primary) + **OpenWeatherMap Geocoding** (fallback) | Free fuzzy matching for cities, zips, landmarks, coordinates; fallback keeps search working when Nominatim throttles the shared host IP |
...
**Alternative: Node.js reference backend** (same port, same API; the frontend works unchanged):
```bash
cd backend-node
npm install
npm start
```
Run either backend, not both at once (they share port 4000).
...
| PUT | `/api/records/:id` | **UPDATE** : revalidates; refetches temps if location/range changed |
...
- Updates: same validation; `temperature_data` is always rederived from the APIs when location/dates change, so stored weather can never contradict the stored location. Users edit location/dates/notes, not raw temperatures.
...
- Weather requests go through a retry with backoff layer and a 10 minute in memory cache; if the primary provider fails, the backend transparently switches to OpenWeatherMap and normalizes its data into the same response shape, so the frontend never changes. Geocoding follows the same pattern: Nominatim primary with a 1 hour cache, OpenWeatherMap Geocoding API as fallback (including US zip and reverse lookups). No external dependency is a single point of failure.
...
backend/                   Python / FastAPI (primary)
  ...
  services/geocode.py      Nominatim geocoding + cache + OWM geocoding fallback
...
backend-node/              Node.js / Express reference implementation of the same API
  ...
  db.js                    SQLite (better sqlite3) schema + connection
  services/                geocode, weather (retry/cache), openweather (fallback), insights
  routes/                  weather, records (CRUD), export, extras
...
5. **Geocoding throttled on the shared IP.** A day after the weather provider incident, Nominatim searches began failing on the deployed backend for the same underlying reason: Nominatim enforces strict per IP fair use limits, and the shared free tier IP was throttled. Fix: applied the same resilience pattern to geocoding a 1 hour result cache and an automatic fallback to the OpenWeatherMap Geocoding API (direct, US zip, and reverse endpoints), reusing the key already configured for the weather fallback.
...
Node reference backend:
```bash
cd backend-node
npm test
```

Each suite has 17 tests covering date range validation (format, impossible dates like Feb 31, range limits, archive/forecast windows), GPS coordinate parsing, the insights engine, and the fallback provider's condition code mapping. Keeping both suites identical is how the Python port was verified against the original Node implementation.
